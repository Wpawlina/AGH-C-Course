...
void mean_variance(const int v[], int n, double *mean, double *variance) {
	...
	for(int i=0;i<n;i++)
	{
		*variance+=(double)((v[i]-*mean)*(v[i]-*mean))/n;
	}
...
void bernoulli_gen(int v[], int n, double probability) {
	...
	int limit=probability*(RAND_MAX);
	for(int i=0;i<n;i++)
	{
		int num=rand();
		if(num<limit)
...
void pmf(double v[], int trials) {
	for(int i=0;i<trials;i++)
	{
		int dice1=1+rand()%6;
		int dice2=1+rand()%6;
		v[dice1+dice2-2]+=(double)1/trials;
	}
}
...
void monty_hall(int n, int *p_switch_wins) {
	...
	{
		int winDoors=rand()%3;
		int chosenDoors=rand()%3;
		if(winDoors!=chosenDoors)
		{
			(*p_switch_wins)++;
		}
	}
